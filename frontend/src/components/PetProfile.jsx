    ...
    <div style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <h3 style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: 22, color: "#1a1a2e", marginBottom: 6 }}>
            {step === 0 ? "Name Your Pet" : "Choose Species"}
          </h3>
          <p style={{ fontFamily: "mono", fontSize: 11, color: "rgba(26,26,46,0.35)" }}>
        ...
        {step === 0 ? (
          <>
            <input value={name} onChange={e => setName(e.target.value)}
              ...
              autoFocus
              style={{
                width: "100%", padding: "14px 18px", borderRadius: 12, background: "rgba(0,0,0,0.02)",
                border: "1px solid rgba(0,0,0,0.08)", color: "#1a1a2e", fontFamily: "'Space Grotesk',sans-serif",
            ...
            <button onClick={() => name.trim() && setStep(1)} disabled={!name.trim()} style={{
              ...
              background: name.trim() ? "linear-gradient(135deg,#f59e0b,#d97706)" : "rgba(0,0,0,0.04)",
              color: name.trim() ? "white" : "rgba(26,26,46,0.25)", fontFamily: "mono", fontSize: 13, fontWeight: 600,
          ...
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 20 }}>
              {PET_SPECIES.map((s, i) => (
                <button key={s} onClick={() => setSpecies(i)} style={{
                  background: species === i ? "rgba(251,191,36,0.1)" : "rgba(0,0,0,0.02)",
                  border: species === i ? "2px solid rgba(251,191,36,0.4)" : "1px solid rgba(0,0,0,0.06)",
                  ...
                  <div style={{
                    ...
                    color: species === i ? "#b45309" : "rgba(26,26,46,0.35)",
                  }}>{s}</div>
                </button>
              ))}
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setStep(0)} style={{
                ...
                color: "rgba(26,26,46,0.5)", fontFamily: "mono", fontSize: 13,
              }}>
                Back
              </button>
  ...
  return (
    <div style={{ position: "relative", display: "inline-block" }}>
      ...
      <div style={{
        ...
        fontSize: size * 0.22, background: "white",
...
export default function PetProfile() {
  ...
  const handleInteract = async (type) => {
    ...
    try {
      ...
      // Simulate stat changes for mock interactions
      setPetStatus(prev => {
        if (!prev) return prev;
        const changes = mockResp.stat_changes;
        return {
          ...prev,
          happiness: Math.min(100, Math.max(0, prev.happiness + (changes.happiness || 0))),
          energy: Math.min(100, Math.max(0, prev.energy + (changes.energy || 0))),
          hunger: Math.min(100, Math.max(0, prev.hunger + (changes.hunger || 0))),
          bond_level: Math.min(100, Math.max(0, prev.bond_level + (changes.bond || 0))),
          experience: prev.experience + (changes.experience || 0),
      ...
      <div style={{ display: "grid", gridTemplateColumns: "340px 1fr", gap: 20 }}>
        ...
        <div style={{
          ...
          <div style={{ textAlign: "center", marginBottom: 24, position: "relative" }}>
            ...
            <h2 style={{
              fontFamily: "'Space Grotesk',sans-serif", fontSize: 26, color: "#1a1a2e",
