...
export default function Error({ error, reset }: { error: Error; reset: () => void }) {
  return (
    <div style={{ padding: 40, fontFamily: "monospace", background: "#1a1a2e", color: "#fff", minHeight: "100vh" }}>
      <h1 style={{ color: "#ff6b6b", marginBottom: 16 }}>Client Error</h1>
      <pre style={{ whiteSpace: "pre-wrap", wordBreak: "break-all", background: "#111", padding: 20, borderRadius: 8, marginBottom: 16 }}>
        {error.message}
      </pre>
      <pre style={{ whiteSpace: "pre-wrap", wordBreak: "break-all", background: "#111", padding: 20, borderRadius: 8, fontSize: 13, color: "#888", marginBottom: 16 }}>
        {error.stack}
      </pre>
