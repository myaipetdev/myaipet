...
// Extension privacy guarantees as scannable chips — the full guarantee text
// lives in each chip's title tooltip (demoted, not deleted).
const EXT_PRIVACY: { label: string; tip: string; icon: React.ReactNode }[] = [
  {
    label: "Allow-list only",
    tip: "PetClaw runs only on sites you explicitly allow.",
    icon: (
  ...
  {
    label: "Sensitive domains blocked",
    tip: "MY AI PET, private/local network addresses, and common sensitive domains are always blocked.",
    icon: (
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
  ...
  {
    label: "Summaries ask twice",
    tip: "Page summaries show the exact excerpt and require a second confirmation before anything is sent.",
    icon: (
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        <path d="m2 13 4 4 7-8" /><path d="m11 15.5 2.5 2.5L22 9" />
      </svg>
    ),
  },
];
    ...
    <div style={{ maxWidth: 760, margin: "0 auto", padding: "104px 20px 56px" }}>
      ...
      {cta && (
        <div style={{
          ...
          <div style={{ flex: "1 1 280px", minWidth: 220 }}>
            <div style={{ fontFamily: "var(--ed-m)", fontSize: 14, fontWeight: 700, letterSpacing: "0.14em", color: "#FCE9CF", marginBottom: 5 }}>
              PETCLAW CONSOLE
            </div>
            ...
            {ctaNote && (
              <div style={{ fontFamily: "var(--ed-body)", fontSize: 14, lineHeight: 1.5, color: "#FCE9CF", marginTop: 5 }}>
                {ctaNote}
              </div>
            )}
          </div>
      ...
      <Reveal dir="fade">
        <div style={{
          ...
          <div style={{ position: "relative" }}>
            <div style={{ fontFamily: "var(--ed-m)", fontSize: 14, fontWeight: 700, letterSpacing: "0.14em", color: "#E8C77E", marginBottom: 12 }}>
            ...
            <p style={{ fontFamily: "var(--ed-body)", fontSize: 15.5, color: "rgba(255,248,238,0.82)", maxWidth: 520, margin: 0, lineHeight: 1.6 }}>
              Most AI forgets you the moment the tab closes — and owns whatever it learns. Here, the more you talk, the closer your pet grows — and its memory <strong style={{ color: "#FFF8EE" }}>stays yours</strong>: <strong style={{ color: "#FFF8EE" }}>exportable, deletable, inheritable</strong>.
            </p>
          </div>
        </div>
      </Reveal>
        ...
        {PILLARS.map((p, i) => (
          <Reveal key={p.title} dir="up" delay={i * 90} style={{ height: "100%" }}>
            <div style={{
              ...
              <div style={{ fontFamily: "var(--ed-m)", fontSize: 14, fontWeight: 700, letterSpacing: "0.12em", color: "#9A7B4E", marginBottom: 4 }}>{p.eyebrow}</div>
              <div style={{ fontFamily: "var(--ed-disp)", fontWeight: 800, fontSize: 15, color: "#211A12", letterSpacing: "-0.01em" }}>{p.title}</div>
              <div style={{ fontFamily: "var(--ed-body)", fontSize: 14, color: "#5C5140", marginTop: 5, lineHeight: 1.5 }}>{p.body}</div>
            </div>
          </Reveal>
        ))}
      </div>
      ...
      <Reveal dir="up">
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 8 }}>
            ...
            <span style={{ fontFamily: "var(--ed-m)", fontSize: 14, fontWeight: 700, color: "#9A4E1E" }}>
              v{PETCLAW_EXTENSION_VERSION} · CHROME
            </span>
          </div>
          <p style={{ margin: "0 0 10px", fontFamily: "var(--ed-body)", fontSize: 14, lineHeight: 1.6, color: "#5C5140" }}>
            Install first, then pair your pet after signing in.
          </p>
          {/* Privacy guarantees — paper-tile chips; hover a chip for the full text. */}
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, margin: "0 0 14px" }}>
            {EXT_PRIVACY.map((c) => (
              <span
                key={c.label}
                title={c.tip}
                style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: 999, background: "#F5EFE2", border: "1px solid rgba(33,26,18,.1)", fontFamily: "var(--ed-body)", fontSize: 14, fontWeight: 600, color: "#5C5140", cursor: "help" }}
              >
                <span aria-hidden="true" style={{ display: "inline-flex", color: "#9A4E1E" }}>{c.icon}</span>
                {c.label}
              </span>
            ))}
          </div>
            ...
            {PETCLAW_EXTENSION_STEPS.map((step) => (
              <li key={step.n} style={{ padding: 12, borderRadius: 12, background: "#F5EFE2", border: "1px solid rgba(33,26,18,.1)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
                  <span aria-hidden="true" style={{ width: 24, height: 24, borderRadius: "50%", display: "inline-flex", alignItems: "center", justifyContent: "center", background: "#BE4F28", color: "#FFF8EE", fontFamily: "var(--ed-m)", fontSize: 14, fontWeight: 700 }}>{step.n}</span>
                  <strong style={{ fontFamily: "var(--ed-disp)", fontSize: 14, color: "#211A12" }}>{step.title}</strong>
                </div>
                <div style={{ fontFamily: "var(--ed-body)", fontSize: 14, lineHeight: 1.5, color: "#5C5140" }}>{step.desc}</div>
              </li>
            ))}
          </ol>
      ...
      <Reveal dir="up">
        <div style={{ marginTop: 16, background: "#F5EFE2", borderRadius: 18, padding: "20px 22px", border: "1px solid var(--ed-hair, rgba(33,26,18,.13))" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: 14, fontFamily: "var(--ed-m)", fontWeight: 700, letterSpacing: "0.08em", padding: "3px 10px", borderRadius: 999, background: "rgba(26,126,104,0.12)", color: "#1A7E68" }}>PUBLIC DISCOVERY</span>
            <div style={{ flex: 1 }} />
            <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontFamily: "var(--ed-m)", fontSize: 14, color: "#1A7E68", fontWeight: 700 }}>
              <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#1A7E68" }} />
              {publicNodes.length} public
            </span>
          </div>
          {visibleNodes.length === 0 ? (
            <div style={{ padding: 16, textAlign: "center", fontFamily: "var(--ed-body)", fontSize: 14, color: "#9A7B4E" }}>
              No public pets yet.
            </div>
          ) : (
              ...
              {visibleNodes.map((n: any) => (
                <div key={n.petId} style={{ display: "flex", alignItems: "center", gap: 12, padding: "9px 12px", borderRadius: 12, background: "#FBF6EC", border: "1px solid var(--ed-hair, rgba(33,26,18,.13))" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontFamily: "var(--ed-m)", fontSize: 14, color: "#7A6E5A", marginTop: 1 }}>
                      {[n.personality, n.element, n.level != null ? `Lv.${n.level}` : null].filter(Boolean).join(" · ")}
                    </div>
                  </div>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 14, padding: "4px 10px", borderRadius: 999, background: "rgba(26,126,104,0.1)", border: "1px solid rgba(26,126,104,0.25)", color: "#1A7E68", fontFamily: "var(--ed-m)", fontWeight: 700 }}>
      ...
      <Reveal dir="up">
        <div style={{ marginTop: 18, padding: "20px 22px", borderRadius: 18, textAlign: "center", background: "#1E1710", color: "#FFF8EE", boxShadow: "var(--ed-shadow-card, 0 20px 40px -26px rgba(80,55,20,.5))" }}>
          ...
          <div style={{ fontFamily: "var(--ed-body)", fontSize: 14, color: "rgba(255,248,238,0.7)", marginBottom: 16 }}>
